...
public class CuckooHash<K, V> {
	...
	/*
	 * pass the key into the 1st hash function to get the location in the first table
	 * check if that location is empty, store the key there and exit
	 */
 	public void put(K key, V value) {
		...
		int pos = hash1(key);
		if (table[pos] != null && table[pos].getBucKey().equals(key) && table[pos].getValue().equals(value)) {
			return;
		}

		Bucket<K, V> buck = new Bucket<K, V>(currentKey, currentValue);
		...
		while (i < CAPACITY) {
			...
			Bucket<K, V> remove = table[pos];
			table[pos] = buck;

			if (pos == hash1(remove.bucKey)) {
				pos = hash2(remove.bucKey);
			} else {
				pos = hash1(remove.bucKey);
			}
